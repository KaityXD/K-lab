...
local jumpTimer = 0
local jumpInterval = 0.8 -- Seconds between each jump
local jumpSpeed = 15
...
function onUpdate(dt)
    ...
    if targetPos.x > 12 or targetPos.x < -12 then targetVel.x = -targetVel.x end
    if targetPos.y > 10 or targetPos.y < 1 then targetVel.y = -targetVel.y end
