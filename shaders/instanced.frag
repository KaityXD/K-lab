...
void main() {
    ...
    vec3 baseColor = pow(InstanceColor, vec3(2.2));
    ...
    if (currentTiling) {
        ...
        if (absNorm.x > 0.9) {
            uv.x *= InstanceObjectScale.z;
            uv.y *= InstanceObjectScale.y;
        } else if (absNorm.y > 0.9) {
            uv.x *= InstanceObjectScale.x;
            uv.y *= InstanceObjectScale.z;
        } else {
            uv.x *= InstanceObjectScale.x;
            uv.y *= InstanceObjectScale.y;
        }
